...
import { startClaudeSession, interuptRunningProcessWithNewComment } from './claude.mjs';
...
async function sendCommentToSession(sessionInfo, newCommentText) {
  try {
    const newProcess = await interuptRunningProcessWithNewComment(sessionInfo.process, newCommentText)
    sessionInfo.process = newProcess
    console.log(
      `✅ Comment successfully sent to Claude for issue ${sessionInfo.issue.id}`
    )
    console.log(
    ...
    try {
      await createComment(
        sessionInfo.issue.id,
        `Agent encountered an error trying to process your comment: ${err.message}`
      )
    } catch (commentError) {
      console.error(
        `Failed to post error comment to Linear for issue ${sessionInfo.issue.id}:`,
        commentError
      )
    }
  }
...
async function startLinearAgent() {
  ...
  try {
    ...
    await fetchAssignedIssuesAndRun()
 ...
 */
async function fetchAssignedIssuesAndRun() {
  try {
    console.log(
      `Fetching issues assigned to user ${process.env.LINEAR_USER_ID}...`
    )
    ...
    for (const issue of issueNodes) {
      await initializeIssueSession(issue)
    }
  } catch (error) {
 ...
 */
async function initializeIssueSession(issue) {
  ...
  try {
    ...
    if (activeSessions.has(issue.id)) {
      console.log(
        `Already have an active session for issue ${issue.identifier}`
      )
      return
    }
    ...
    activeSessions.set(issue.id, {
      issue,
      workspacePath,
      historyPath: getHistoryFilePath(workspacePath),
    ...
    claudeProcess.on('exit', async (code) => {
      ...
      if (session && session.process) {
        ...
        if (code !== 0) {
          console.error(
            `Claude process for ${issue.identifier} exited abnormally (code ${code}).`
          )
          let errorMessage = `Claude process for issue ${issue.identifier} exited unexpectedly with code ${code}.`
          if (stderrContent) {
            errorMessage += `\n\n**Error details (stderr):**\n\`\`\`\n${stderrContent.substring(
              0,
              1500
            )} ${stderrContent.length > 1500 ? '... (truncated)' : ''}\n\`\`\``
          }
          try {
            await createComment(issue.id, errorMessage)
          } catch (commentError) {
            console.error(
              `Failed to post error exit status comment to Linear for issue ${issue.identifier}:`,
              commentError
            )
          }
        } else {
          console.log(
            `Claude process for ${issue.identifier} finished successfully. Final messages were posted during the run.`
          )
        }
        ...
        console.warn(
          `Could not find active session or process details for issue ${issue.id} upon exit.`
        )
      }
    })
  } catch (error) {
    ...
    try {
      ...
      console.error(
        `Failed to post error comment to Linear for issue ${issue.identifier}:`,
        commentError
      )
    }
  }
  ...
  try {
    ...
    if ('assigneeId' in issueData) {
      ...
      if (activeSessions.has(issueData.id)) {
        ...
        if (
          previousAssigneeId === process.env.LINEAR_USER_ID &&
          (newAssigneeId === null ||
            newAssigneeId !== process.env.LINEAR_USER_ID)
        ) {
          console.log(
            `Issue ${sessionInfo.issue.identifier} has been unassigned from our agent, terminating Claude process`
          )
          ...
          if (sessionInfo.process && !sessionInfo.process.killed) {
            try {
              ...
              console.log(
                `✅ Terminated Claude process for issue ${sessionInfo.issue.identifier}`
              )
            } catch (killError) {
              console.error(
                `Error terminating Claude process for issue ${sessionInfo.issue.identifier}:`,
                killError
              )
            }
          } else {
            console.log(
              `No active Claude process to kill for issue ${sessionInfo.issue.identifier}`
            )
          }
        }
      }
      // If the issue is newly assigned to our agent
        ...
        console.log(
          `Issue ${
            issueData.identifier || issueData.id
          } has been newly assigned to our agent, starting Claude process`
        )
...
async function handleCommentEvent(event) {
  try {
    ...
    if (!body.includes(process.env.LINEAR_USERNAME)) {
      console.log(
        `Skipping comment that does not mention the agent (${process.env.LINEAR_USERNAME})`
      )
      return
    }
    ...
    console.log(
      `===== WEBHOOK: Received comment on assigned issue ${issueId} that mentions ${process.env.LINEAR_USERNAME}. So processing... =====`
    )
    ...
    if (sessionInfo && sessionInfo.process) {
      ...
      console.error(
        `Received comment on assigned issued, yet could not find valid session for the issue ${issueId}. Doing nothing.`
      )
    }
  } catch (error) {
